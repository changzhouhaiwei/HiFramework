...
namespace xasset
{
    ...
    {
        public const string APIVersion = "2022.1.3p6";
        ...
        public static ClearFiles ClearAsync(params string[] files)
        {
            ...
            if (files.Length == 0)
            {
                if (Directory.Exists(Downloader.DownloadDataPath))
                {
                    ...
                    var usedFiles = new HashSet<string> { Filename };
        ...
        /// <returns></returns>
        public static GetDownloadSize GetDownloadSizeAsyncWithManifests(params string[] manifests)
        {
            var getDownloadSize = new GetDownloadSize();
            foreach (var manifest in manifests)
            {
        ...
        private static void GetBundles(ICollection<ManifestBundle> bundles, Manifest manifest, string assetPath)
        {
            ...
            if (assets != null && assets.Count != 0)
            {
                ...
                foreach (var asset in assets)
                {
                    if (manifest.IsDirectory(asset))
                    {
                        ...
                        {
                            GetBundles(bundles, manifest, child);
                        }
                    }
                    else
                    {
                        ...
                        GetBundles(bundles, manifest, actualPath);
                    }
                }
            }
            else
